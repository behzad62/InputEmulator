...
namespace KeyboardEmuAPI
{
    static class NativeMethods
    {
        ...
        [DllImport("kernel32", CharSet = CharSet.Unicode, SetLastError = true)]
        public static extern bool DeviceIoControl(SafeFileHandle hDevice, uint controlCode, IntPtr request,
            uint inputSize, out KEYBOARD_ATTRIBUTES result, uint outputSize, out uint returned, IntPtr lpOverlapped);

        [DllImport("kernel32", CharSet = CharSet.Unicode, SetLastError = true)]
        public static extern bool DeviceIoControl(SafeFileHandle hDevice, uint controlCode, KEYBOARD_INPUT_DATA[] inputKeys, uint inputSize,
            IntPtr result, uint outputSize, out uint returned, IntPtr lpOverlapped);
